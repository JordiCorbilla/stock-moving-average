...
def PlotCumulativeMovingAverage(ticker, start_date, project_folder):
    ...
    # EMA - Exponential Moving Average with Smooth factor of 1%
    data['EMA1'] = data.average_close_price.ewm(alpha=0.01, adjust=False).mean()
    data.to_csv(os.path.join(project_folder, 'calculated_data' + ticker + '.csv'))

    plt.figure(figsize=(12, 5))
    plt.plot(data.Close, color='green')
    plt.plot(data.EMA1, color='red')
    plt.ylabel('Price [' + sec.info['currency'] + ']')
    plt.xlabel("Date")
    plt.title(sec.info['shortName'])
    plt.legend(["["+STOCK_TICKER+"] Close Price", "EMA - 1%"])
